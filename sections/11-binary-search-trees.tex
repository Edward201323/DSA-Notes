...
This property makes a BST behave like a sorted array, allowing us to eliminate half of the tree at every step during search. Assuming that a BST is perfectly balanced, searching, inserting and deleting are all $O(\log n)$ operations. Meanwhile, arrays require $O(n)$ shifts for insert/delete.
...
Searching in a balanced BST has a time complexity of \textbf{$O(\log n)$} while searching in an unbalanced tree is $O(n)$ as it has degenerated into a linked list.
...
\textbf{Case 1: Leaf node (no children)} --- simply remove and replace with \texttt{null}.

\textbf{Case 2: Node with one child} --- return the non-null child.

\textbf{Case 3: Node with two children} --- replace the node with the smallest node in the right subtree (in-order successor), then delete that leaf.

\textbf{Example: delete node 4}

Step 1: find successor (node 5, smallest in right subtree)

Step 2: replace 4 with 5 and delete the original leaf 5.
...
Prints values in sorted order for a BST: $2, 3, 4, 5, 6, 7$
...
Processes the root before its subtrees: $4, 3, 2, 6, 5, 7$
...
Processes children before the parent: $2, 3, 5, 7, 6, 4$
...
Using the same tree, BFS prints: $4 \rightarrow 3,6 \rightarrow 2,5,7$
