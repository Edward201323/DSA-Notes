...
public class DepthFirstPaths {
    ...
    Vertex input;
    public DepthFirstPaths(Graph graph, Vertex input) {
        visited = new HashSet<>();
        edgeTo = new HashMap<>();
        this.input = input;
        dfs(graph, input);
    ...
    private void dfs(Graph graph, Vertex current) {
        visited.add(current); // Mark current vertex as visited
        for (Vertex neighbor : graph.adj(current)) { // Check each neighbor
            if (!visited.contains(neighbor)) {
                edgeTo.put(neighbor, current); // Record that we reached neighbor from current
                dfs(graph, neighbor); // Recurse into unvisited neighbor
            }
        }
    }
}
...
public class CountPaths {
    ...
    Set<Vertex> visited;
    public CountPaths(Map<Vertex, List<Vertex>> adjList) {
        this.adjList = adjList;
        this.visited = new HashSet<>();
    }
    public int dfs(Vertex node, Vertex target) {
        if (visited.contains(node)) return 0; // Already on this path
        if (node.equals(target)) return 1; // Reached target, count this path
        int count = 0;
        visited.add(node); // Add to current path
        for (Vertex neighbor : adjList.get(node)) {
            count += dfs(neighbor, target); // Explore each neighbor
        }
...
public class ShortestPath {
    Map<Vertex, List<Vertex>> adjList;
    Set<Vertex> visited;
    public ShortestPath(Map<Vertex, List<Vertex>> adjList) {
        this.adjList = adjList;
        this.visited = new HashSet<>();
    }
    public int bfs(Vertex node, Vertex target) {
        int length = 0;
        visited.add(node);
        Queue<Vertex> queue = new LinkedList<>();
        queue.add(node); // Start with source in queue
        while (!queue.isEmpty()) {
            int size = queue.size(); // Number of nodes at current level
            for (int i = 0; i < size; i++) {
                Vertex curr = queue.peek(); // Process next node in level
                queue.remove();
                if (curr.equals(target)) return length; // Found target
                for (Vertex neighbor : adjList.get(curr)) {
                    if (!visited.contains(neighbor)) {
                        visited.add(neighbor); // Mark to avoid revisiting
                        queue.add(neighbor); // Add to next level
                    }
                }
            }
            length++; // Move to next level
        }
